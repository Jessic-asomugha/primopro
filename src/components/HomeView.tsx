  ...
  const steps = [
    {
      ...
      desc: 'Deploying high-precision seismic models, offshore rigs, and core testing to access oil and gas reserves safely.'
    },
    {
      number: '02',
      title: 'Refining & Chemical Synthesis',
      desc: 'Optimizing crude distillation to synthesize high-purity petrochemical feedstocks and ultra-clean commercial fuels.'
    },
    {
      number: '03',
      title: 'Global Transport & Pipelines',
      desc: 'Utilizing leak-proof smart pipelines and double-hull cargo tanker fleets to secure global energy delivery.'
    }
  ];
    ...
    <div className="bg-white text-brand-dark overflow-hidden">
      ...
      <section className="relative min-h-[88vh] flex items-center bg-brand-light">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            ...
            >
              <div className="inline-flex items-center gap-2 px-3.5 py-1.5 bg-brand-accent/10 rounded-full text-brand-accent text-xs font-semibold uppercase tracking-wider border border-brand-accent/20">
                ...
                Global Oil & Gas Leaders
              </div>
              
              <h1 className="font-display text-4xl sm:text-5xl lg:text-6xl font-bold tracking-tight text-brand-dark leading-[1.1]">
                Primo Energy. <br />
                <span className="text-brand-accent">Oil & Power Solutions.</span>
              </h1>
              
              <p className="text-base sm:text-lg text-brand-muted leading-relaxed">
                Primo pioneers safe hydrocarbon extraction, high-yield refining capabilities, and reliable global transport, while aggressively investing in cleaner transitional low-carbon energy.
              </p>
        ...
        <div className="absolute bottom-0 right-0 left-0 lg:left-1/2 bg-brand-dark text-white py-4 z-25 overflow-hidden border-t border-white/10">
          <div className="max-w-7xl mx-auto px-4 flex flex-wrap gap-x-8 gap-y-2 text-xs font-mono font-medium tracking-wider text-gray-300">
            <span className="flex items-center gap-1 text-brand-accent">✦ Upstream Exploration</span>
            <span className="flex items-center gap-1">✦ Downstream Refining</span>
            <span className="flex items-center gap-1">✦ Midstream Logistics</span>
            <span className="flex items-center gap-1 text-brand-accent">✦ Sustainable Transition</span>
          </div>
        </div>
      </section>
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          >
            ...
            <div className="space-y-4 lg:pr-4">
              ...
              <h2 className="font-display text-3xl font-bold tracking-tight">Supply Integration</h2>
              <p className="text-sm text-gray-400 leading-relaxed">
                Through operational optimization, our integrated supply chain secures hydrocarbon flows from original geological reservoir to regional fuel terminals.
              </p>
              <button 
                onClick={onNavigateToAbout}
                className="inline-flex items-center gap-1 text-sm font-semibold text-brand-accent hover:text-white transition group pt-2"
              >
                Learn Corporate Strategy
          ...
          >
            ...
            <div className="lg:col-span-5 relative">
              ...
              <div className="absolute -bottom-6 -right-6 bg-brand-accent text-white p-6 rounded-xl shadow-xl hidden sm:block max-w-xs">
                <span className="block font-mono text-xs uppercase tracking-widest text-white/80">HSE Standard</span>
                <span className="block text-3xl font-display font-bold mt-1">100% Safety</span>
                <span className="block text-xs text-white/80 mt-1">Zero recordable incident mandate across offshore & onshore units.</span>
              </div>
            </div>

            {/* Editorial Content Column */}
            <div className="lg:col-span-7 space-y-6 lg:pl-6">
              <span className="block text-xs font-bold uppercase tracking-widest text-brand-accent">Enterprise Mission</span>
              ...
              <div className="space-y-4">
                <div className="border-l-4 border-brand-accent pl-4">
                  <h3 className="font-display text-lg font-semibold text-brand-dark">Securing Peak Hydrocarbon Yield</h3>
                  <p className="text-sm text-brand-muted mt-1 leading-relaxed">
                    Primo enforces peak operational efficiency across extraction and distillation. By optimizing logistics networks, we yield consistent fuel supply while maintaining zero-incident environmental safety.
                  </p>
                </div>

                <div className="border-l-4 border-brand-accent pl-4">
                  <h3 className="font-display text-lg font-semibold text-brand-dark">Investing in Transitional Bio-fuels</h3>
                  <p className="text-sm text-brand-muted mt-1 leading-relaxed">
                    Primo invests heavily in carbon capture and storage (CCS), next-generation hydrogen fuel matrices, and renewable solar power arrays to secure clean future energy.
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2 text-xs font-semibold text-brand-dark">
                <div className="flex items-center gap-2 bg-brand-light px-3 py-2 rounded-lg">
                  ...
                  Mission: Global Power
                </div>
                <div className="flex items-center gap-2 bg-brand-light px-3 py-2 rounded-lg">
                  <CheckCircle2 className="h-4 w-4 text-brand-accent" />
                  Vision: Clean Transition
                </div>
                <div className="flex items-center gap-2 bg-brand-light px-3 py-2 rounded-lg">
                  <CheckCircle2 className="h-4 w-4 text-brand-accent" />
                  Values: Safety & Excellence
                </div>
              </div>
      ...
      <section className="py-24 bg-brand-light">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          >
            <span className="block text-xs font-bold uppercase tracking-widest text-brand-accent">Our Operational Divisions</span>
            <h2 className="font-display text-3xl sm:text-4xl font-bold tracking-tight text-brand-dark">
              Meeting World Energy Demand
            </h2>
            <p className="text-sm text-brand-muted leading-relaxed">
              Leading global operations across upstream deepwater exploration, midstream transport logistics, and downstream refining.
            </p>
          </motion.div>
